  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      row = await req.runInTenant!(async (db) => {
        // Safe auto-migration check for new columns
        ...
        const [emp] = await db.insert(employees).values({
          employeeCode,
          firstName: d.firstName || 'Employee',
          lastName: d.lastName || '',
          email: d.email || null,
          phone: d.phone || null,
          dob: d.dob || null,
          ...
          grade: d.grade || null,
          costCentre: d.costCentre || null,
          uanNumber: d.uanNumber || null,
          esicIpNumber: d.esicIpNumber || null,
          ...
          probationDays: d.probationDays && !isNaN(Number(d.probationDays)) ? Number(d.probationDays) : null,
          bankAccountName: d.bankAccountName || null,
          bankAccountType: d.bankAccountType || null,
          address: d.address || null,
          priorExperienceMonths: d.priorExperienceMonths && !isNaN(Number(d.priorExperienceMonths)) ? Number(d.priorExperienceMonths) : null,
          shiftStartTime: d.shiftStartTime || null,
          shiftEndTime: d.shiftEndTime || null,
